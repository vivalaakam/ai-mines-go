...
// TestGamepadMapCursorSelectsWorker exercises the map cell-cursor A action:
// with the cursor on a worker's cell, mapCursorAction selects it via the same
// handleWorkerClick flow the mouse uses.
func TestGamepadMapCursorSelectsWorker(t *testing.T) {
	...
	// positionCellId is "x,y"; parse it into cursor coordinates.
	var cx, cy float64
